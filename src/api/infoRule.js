export default function RULE() {
  ...
  const numberREG10 =/^(0.\d+|0|1)$/; //数字
  const positiveNumber =/(^[1-9](\d+)?(\.\d{1,2})?$)|(^\d\.\d{1,2}$)/; //正数（包括小数）
  ...
  return {
    ...
    validateNum: (rule, value, callback) => {   //验证课时包数字
      if (value === '') {
        callback(new Error('请输入课时包'));
      }
    ...
    validateNumber: (rule, value, callback) => {   //验证数字
    ...
    validateNumber10: (rule, value, callback) => {   //验证1-0之间数字
      if (value&&!numberREG10.test(value)) {
        callback(new Error('输入1-0之间数字'));
      }
      else {
        callback();
      }
    },
    lowest_class_num: (rule, value, callback) => {   //最低购买课时数
      if (value === '') {
        callback(new Error('请输入最低购买课时数'));
      }
      else if (!numberREG.test(value)) {
        callback(new Error('请输入大于0的整数'));
      }
    ...
    positiveNumber: (rule, value, callback) => {   //最低购买课时数
      if (value === '') {
